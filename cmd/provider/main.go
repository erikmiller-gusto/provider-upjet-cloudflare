...
import (
	...
	apisCluster "github.com/crossplane-contrib/provider-upjet-cloudflare/apis/cluster"
	apisNamespaced "github.com/crossplane-contrib/provider-upjet-cloudflare/apis/namespaced"
	"github.com/crossplane-contrib/provider-upjet-cloudflare/config"
	"github.com/crossplane-contrib/provider-upjet-cloudflare/internal/clients"
	controllerCluster "github.com/crossplane-contrib/provider-upjet-cloudflare/internal/controller/cluster"
	controllerNamespaced "github.com/crossplane-contrib/provider-upjet-cloudflare/internal/controller/namespaced"
	"github.com/crossplane-contrib/provider-upjet-cloudflare/internal/features"
	"github.com/crossplane-contrib/provider-upjet-cloudflare/internal/version"
)
...
func main() {
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-cloudflare"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-cloudflare",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Cloudflare APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Cloudflare APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Cloudflare controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Cloudflare controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Cloudflare controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Cloudflare controllers")
	}
